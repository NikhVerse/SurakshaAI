...
const NAV_ITEMS: NavItem[] = [
  { name: "Dashboard", href: "/app/dashboard", icon: LayoutDashboard, shortcut: "⌘D" },
  { name: "Triage Queue", href: "/app/triage", icon: AlertTriangle, shortcut: "⌘T" },
  { name: "Incidents", href: "/app/reports", icon: FileText, shortcut: "⌘I" },
  { name: "Precursors", href: "/app/precursors", icon: Layers, shortcut: "⌘P" },
  { name: "Barrier Health", href: "/app/barriers", icon: ShieldCheck, shortcut: "⌘B" },
  { name: "Data Stream", href: "/app/data-feed", icon: Database, shortcut: "⌘F" },
  { name: "Audit Trail", href: "/app/audit-log", icon: FileCheck2, shortcut: "⌘A" },
  { name: "Documentation", href: "/docs", icon: BookOpen, shortcut: "⌘?" },
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <div key={item.href} className="relative group flex items-center justify-center w-full">
                  <Link
                    href={item.href}
                    className={`relative flex items-center justify-center w-11 h-11 rounded-xl transition-all duration-150 cursor-pointer ${
                      isActive
                  ...
                  {/* Floating Dark Pill Tooltip matching reference screenshot */}
                  <div className="absolute left-[calc(100%+10px)] top-1/2 -translate-y-1/2 pointer-events-none z-50 hidden md:flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#18181b] text-white text-xs font-semibold shadow-2xl opacity-0 translate-x-[-6px] scale-95 group-hover:opacity-100 group-hover:translate-x-0 group-hover:scale-100 transition-all duration-150 ease-out whitespace-nowrap">
                    <span>{item.name}</span>
                    {item.shortcut && (
                      <span className="px-1.5 py-0.5 rounded-md bg-[#27272a] text-neutral-300 text-[10px] font-mono font-medium tracking-normal border border-neutral-700/60">
                        {item.shortcut}
                      </span>
                    )}
                  </div>
